...
(function (FS) {
  ...
  const { escapeHtml: esc, escapeAttr: a, formatCurrency: fC, formatK: fK,
    pickColor, autoTextColor, statusColor, dateToWeek } = FS.utils;
  ...
  function renderHeader() {
    ...
    const nowWeek = FS.constants.NOW_WEEK_INDICATOR;
    ...
    for (let i = 1; i <= 52; i++) {
      ...
      if (i === nowWeek) cls += ' gh-nw';
      w += `<div class="${cls.trim()}">${i}</div>`;
    }
    return `${q}${w}</div></div>`;
  }
  ...
  /** Paletmarkering — gemeenschappelijk voor camp/tactic-modals. */
  function palHTML(selected) {
    let html = '';
    FS.constants.PALETTE.forEach((c) => {
      html += `<div class="pal-sw${c === selected ? ' act' : ''}" style="background:${a(c)}" data-col="${a(c)}"></div>`;
    });
    return `<div class="pal-grid">${html}</div>`;
  }

  FS.render = { render, renderGantt, renderSummary, renderLegend, barHTML, palHTML };
})(window.FS = window.FS || {});
